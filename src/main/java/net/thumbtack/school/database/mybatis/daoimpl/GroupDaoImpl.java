...
public class GroupDaoImpl extends DaoImplBase implements GroupDao {
    ...
    public Group insert(School school, Group group) {
        LOGGER.debug("DAO insert Group {} which in School {}", group, school);
        try (SqlSession sqlSession = getSession()) {
            try {
                getGroupMapper(sqlSession).insert(school, group);
            } catch (RuntimeException ex) {
                LOGGER.info("Can't insert Group {} which in School {}. {}", group, school, ex);
                sqlSession.rollback();
                throw ex;
            }
            sqlSession.commit();
    ...
    public void addSubjectToGroup(Group group, Subject subject) {
        ...
        try (SqlSession sqlSession = getSession()) {
            try {
                ...
                LOGGER.info("Can't add Subject to Group. {}", ex);
                sqlSession.rollback();
                throw ex;
            }
            sqlSession.commit();
